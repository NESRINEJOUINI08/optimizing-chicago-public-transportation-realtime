...
class Station(Producer):
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        #
        #
        # Decide on a topic name, number of partitions, and number of
        # replicas
        #
        #
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        try:
            logger.debug(
                f"Producing arrival event | station_id={self.station_id} "
                f"| train_id={train.train_id} | direction={direction} "
                f"| line={self.color.name} | status={train.status.name} "
            ...
            self.producer.produce(
                ...
                value={
                    ...
                    "direction": direction,
                    "line": self.color.name,            # e.g. "red", "green", "blue"
                    "train_status": train.status.name,   # e.g. "in_service"
                    "prev_station_id": prev_station_id,
                    "prev_direction": prev_direction,
                },
            )
        except Exception as e:
            logger.error(f"Error producing arrival event: {e}")
